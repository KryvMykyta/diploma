<script lang="ts">
	...
	import type { MoviesList, Movie, UserRatingRes } from '~/types';
	...
	const generateMoviesFromRatings = (ratings: UserRatingRes): MoviesList => {
		const movieData: MoviesList = {
			movies: [],
			page: ratings.page.toString(),
			totalPages: ratings.totalPages
		}

		movieData.movies = ratings.userRatings.map((rating) => {
			return {
				...rating.movie,
				ratings: [
					{
	...
	const fetchData = async(page: number) =>{
		isLoading = true;
		const res = await MainProtected.getRatings(page, limit);
		const movieData = generateMoviesFromRatings(res)
		movies = movieData;
		currentPage = page;
		isLoading = false;
	}
